...
async function callWeatherAPI(lat, lng, lang) {
  try {
    ...
    const ariana_w = `
      <h3 class="font-bold text-2xl md:text-4xl font-md">${firstDay.weather[0].description}</h3>
      <p dir="rtl" class="font-bold text-2xl md:text-4xl font-md">${firstDay.temp.max}&deg;C</p>
...
// when click a city in cities list
...
// display cities in every where in website
function populateCities(lang) {
  console.log(lang)
  const citiesList = document.getElementById("citiesList");
  citiesList.innerHTML = "";
  // checking for page language first to populate cities in page language
  if (lang === "ar") {
    arTun.forEach((city) => {
      const cityTemplate = `
            <a href="#" 
...
// to init the map in homepage and call weather api if clicked on a spot on the map
